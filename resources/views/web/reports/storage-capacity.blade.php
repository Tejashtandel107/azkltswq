...
<div class="page-content fade-in-up">
    <div class="ibox mb-2 noprint">
        <div class="ibox-body py-3" style="color: #000">
            <form method="GET" action="{{ route('admin.reports.stock-report.show') }}" id="form-filter">
                <div class="flexbox">
                    <div class="flexbox noprint">
                        <div class="form-group mr-2">                        
                            <label class="pr-1">Chamber:</label>
                            <select name="ch" id="ch" class="form-control" onchange="formsubmit()">
                                <option value="">Select Chamber</option>
                                <option value="all">View All</option>
                                @foreach ($chambers as $key => $value)
                                    <option value="{{ $key }}" @selected(request('ch') == $key)>{{ $value }}</option>
                                @endforeach
                                </select>
                        </div>
                        <div class="form-group">                        
                            <label class="pr-1">Floor:</label>
                            <select name="fl" id="fl" class="form-control" onchange="formsubmit()">
