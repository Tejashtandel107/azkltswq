...
<div class="page-content fade-in-up">
    <div class="ibox mb-2">
        <div class="ibox-body py-3">
            <form action="{{ route('admin.inwards.index') }}" method="GET" id="form-filter" autocomplete="off">
            <div id="notify"></div>
                        ...
                        <div class="mr-2 mb-2">
                           ...
                           <select name="customer_id" class="form-control" onchange="formsubmit()">
                                ...
                                @foreach($customers as $customer)
                                    <option value="{{ $customer->customer_id }}" @selected(request('customer_id') == $customer->customer_id)>
                                        {{ $customer->fullname }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <div class="flexbox mb-2">
                <div class="form-inline">
                    ...
                    <select name="p" class="form-control mr-2" onchange="formsubmit()">
                        <option value="50" {{ ($request->p ?? $show) == '50' ? 'selected' : '' }}>50</option>
                        <option value="100" {{ ($request->p ?? $show) == '100' ? 'selected' : '' }}>100</option>
                        <option value="150" {{ ($request->p ?? $show) == '150' ? 'selected' : '' }}>150</option>
                        <option value="200" {{ ($request->p ?? $show) == '200' ? 'selected' : '' }}>200</option>
                        <option value="300" {{ ($request->p ?? $show) == '300' ? 'selected' : '' }}>300</option>
                    </select>
...
@section('page-scripts')
    <script src="{{ asset('/assets/admin/js/inwards/index.js') }}"></script>
@endsection
