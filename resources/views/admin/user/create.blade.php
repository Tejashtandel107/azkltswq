...
<div class="page-content fade-in-up">
    <div class="row">
        <div class="{{(isset($user)) ? 'col-lg-8' : 'col-lg-12'}}" id="user-details">
            ...
            @if(isset($user))
                <form method="POST" action="{{ route('admin.admins.update', $user->user_id) }}" enctype="multipart/form-data" id="user-form">
                    @method('PATCH')
            @else
                <form method="POST" action="{{ route('admin.admins.store') }}" enctype="multipart/form-data" id="user-form" autocomplete="off">
            @endif
                @csrf
                    <div class="ibox-body">
                        <div id="notify"></div>
    ...
    @if(isset($user))
        <div class="col-md-4" id="changepass">
            <div class="ibox">
                ...
                <div id="notify"></div>
                <form action="{{ route('admin.admins.changepass', $user->user_id) }}" method="POST" id="reset-password-form" name="reset-password-form" autocomplete="off" >
                    @csrf
                    <div class="ibox-body">
...
@section('page-scripts')
    <script src="{{ asset('/assets/admin/js/user/create.js') }}"></script>
    @if (session('type'))
